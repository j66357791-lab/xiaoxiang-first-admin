...
export default function Categories() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      // 🔧 健壮性判断：兼容不同的响应结构
      if (res && res.data) {
        // 如果 request 封装直接返回了 body，则 res 就是 data
        ...
        if (list.length > 0) {
          setCategories(list);
          // 🆕 自动展开所有行，确保数据可见
          const allIds = list.map(item => String(item._id));
  ...
  const buildTree = (data) => {
    ...
    // 🆕 优化：使用 String() 强制转换 ID，防止对象/字符串类型不匹配
    const map = {};
    const roots = [];
    
    // 1. 初始化 Map，ID 统一转为字符串
    data.forEach(item => {
  ...
  const getLevel = (record) => {
    ...
    // 注意：这里也需要确保 ID 比较的一致性
    const parent = categories.find(c => String(c._id) === String(record.parentId));
  ...
  const getParentOptions = () => {
    ...
    const addOptions = (items, level = 1) => {
      items.forEach(item => {
        if (!editingCategory || String(item._id) !== String(editingCategory._id)) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      render: (_, record) => {
        const hasChildren = categories.some(c => String(c.parentId) === String(record._id));
        const level = record.level || getLevel(record);
        return (
          <Space>
